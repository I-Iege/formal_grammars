	...
	for(sign_list::iterator it=g.N.begin();it!=g.N.end();++it)
	{
		...
		{	
            if(it2->size()==1 && it2->begin()->Is_terminal())
			{
				...
				{			
                    for(sign_list::iterator it4=pd.Z.begin();it4!=pd.Z.end();++it4)
					{
...
{
	std::string str="";
    for(std::map<move,std::list<result> >::const_iterator it=pd.M.begin();it!=pd.M.end();++it)
	{
...
std::string gr_to_pd::get_moves() const
{
    return moves[alg_it];
}

 push_down gr_to_pd::get_push_down() const
 {
    return pd;
 }
