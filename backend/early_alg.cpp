	...
	for(int i=0;i<=j;++i)
	{
        ...
        sign_list e_signs_z;
			...
			for(std::set<s_rule>::iterator it=table[i][j].begin(); it!=table[i][j].end();++it)
			{
                ...
                while(((it->get_dot()+z+1)<it->size()) && e_set.count((*it)[it->get_dot()+z])>0 && !((*it)[it->get_dot()+z+1]).Is_terminal());
            }
                        ...
                        for(std::set<s_rule>::iterator it2=table[k][j].begin();it2!=table[k][j].end();++it2)
                        {
                            if((it2->get_n()==b) && (it2->get_dot()==it2->size()))
                            {
                                ...
                                if(table[i][j].count(sr)==0)
                                {
                                    ...
                                    t_info+="#"+std::to_string(i)+"#"+std::to_string(j) +"#"+std::to_string(k);
                                    if(m>0)
                                    {
                                        t_info+="#" + e_signs.to_string();
                                    }
...
early_alg::early_alg(const grammar &gr,const std::vector<sign> &a)
{
    ...
    {
        for(std::set<s_rule>::iterator it=table[i][a.size()].begin();!contains && it!=table[i][a.size()].end();++it)
        {
...
std::string early_alg::to_string() const
{
    return "";
}

early_alg::early_state early_alg::show_state() const
{
    return state[alg_it];
}
