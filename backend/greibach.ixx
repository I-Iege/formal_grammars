...
export module formal_languages:greibach;
...
//inherited from grammar_alg
    std::string to_string() const { return g[alg_it].to_string(); }
    const grammar& get_grammar() const { return g[alg_it]; }

    greibach::greibach_state show_state() const
    {
    ...
    const grammar& get_result() const { return g.back(); }
    ...
    void sort_n()
    {
        ...
        for (int i = 0; i < h.N.size(); ++i)
        {
            ...
            while (it != h.P[a].end())
            {
                ...
                {
                    std::string t_info = "";
                    t_info += a.to_string() + "->" + it->to_string() + "#" + a.to_string() + ">" + b.to_string() + "#"
                              + b.to_string();
                    info.push_back(t_info);
