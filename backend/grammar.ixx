...
export module formal_languages:grammar;
...
public:
    ...
    grammar(const sign_list& n,
                     const sign_list& t,
                     const std::map<sign, std::list<sign_list>>& p,
                     const sign& s)
    {
        this->N = n;
        this->T = t;
    ...
    friend std::ostream& operator<<(std::ostream& s, grammar a)
    {
        s << "kezdojel:"
          << " " << a.S << std::endl;
                ...
                for (std::list<sign_list>::iterator it2 = g.P[*it].begin(); it2 != g.P[*it].end();)
                {
                    if (it2->size() == 1 && !it2->begin()->Is_terminal()) // A->B rule
                    {
                        ...
                        if (s_set.count(b) == 0)
                        {
                            sign_list A;
                            A.push_back(*it);
                            for (std::list<sign_list>::iterator it3 = g.P[b].begin(); it3 != g.P[b].end();
                                 ++it3) // insert B's rules to A's rules
                            {
...
private:
    ...
    void split(const sign str, sign_list& v, const char delim =' ')
    {
     ...
     void make_combination_list(std::vector<std::vector<int>>& store,
    ...
    std::list<sign_list> make_comb_rules(std::vector<int> leave, std::set<int> stay, const sign_list& rule)
    {
    ...
    void replace(const sign& a, const sign& b)
    {
